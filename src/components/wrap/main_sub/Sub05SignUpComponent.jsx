...
export default function Sub05SignUpComponent({회원가입}) {
    ...
    const {
        isId, isIdDuplCheck, isPw1, isPw2, isName, isEmail, isEmailDuplCheck, isHp,isHpDis, isUserHpDis, isAddr1, isAddr2, isGender,  
        isService, msgAddr1, msgAddr2, msgChooga1, msgChooga2, msgEmail, isHpCheck,
        msgGender, msgHp, msgId, msgName, msgPw1, msgBirth, msgPw2, msgService, msgYear, 
        아이디, 비밀번호, 비밀번호확인, 이름, 이메일, 휴대폰, 휴대폰인증발급번호, 휴대폰인증입력번호,
        생년, 생월, 생일, 성별, 추천인아이디, 추가입력사항, 이용약관, 이용약관동의, 이용약관동의필수
    } = state;
    ...
    const onClickDuplCheck=(e, value)=>{
        ...
        if(value==='ID중복확인'){
            ...
            axios({
                url:'http://junelee717.dothome.co.kr/kurly/id_duplicate_check.php',
                method:'post',
                data:newFormData
            })
            .then((res) => {
                if(res.status===200)
                    if(res.data!==1){
                        ...
                        confrimModalOpen('사용 불가능한 아이디입니다.',1);     
                    }
            })
            .catch((err) => {
                console.log(err);
            ...
            axios({
                url:'http://junelee717.dothome.co.kr/kurly/email_duplicate_check.php',
                method:'post',
                data:newFormData
            })
            .then((res) => {
                if(res.status===200)
                    if(res.data!==1){
                        ...
                        confrimModalOpen('중복된 이메일입니다. 이메일 주소를 확인해 주세요.',1);     
                    }
            })
            .catch((err) => {
                console.log(err);
    ...
    const onChangeChooga=(e)=>{
        ...
        if(e.target.value==='친구초대 추천인 아이디'){
            msgChooga1 = '추천인 아이디를 입력해 주세요.';
            msgChooga2 = '가입 후 7일 내 첫 주문 배송완료 시, 친구초대 이벤트 적립금이 지급됩니다.';
        } else {
            msgChooga1 = '참여 이벤트명을 입력해 주세요.';
            msgChooga2 = '추천인 아이디와 참여 이벤트명 중 하나만 선택 가능합니다. 가입 이후는 수정이 불가능 합니다. 대소문자 및 띄어쓰기에 유의해주세요.';
        }
        setState({
            ...state,
            추가입력사항: e.target.value,
            msgChooga1 : msgChooga1,
            msgChooga2 : msgChooga2
        })
    }

    // 추천인 아이디
    const onChangeChoochun=(e)=>{
        setState ({
            ...state,
    ...
    const onClickIDCheck=(e)=>{
        e.preventDefault();
        // db 데이터와 비교 확인
        confrimModalOpen('추천인 아이디가 확인되었습니다. 친구초대 이벤트에 참여 가능해요.');
    }
    ...
    const onSubmitSignup=(e)=>{
        ...
        console.log(result);
        console.log(이용약관동의필수);
        
        // 유효성 검사 : 필수 입력사항이 빈칸인지 아닌지 테스트. 중복확인 등이 완료되었는지 테스트.
        if(아이디===''){
            confrimModalOpen('아이디를 입력해 주세요.');
        } else if (isIdDuplCheck===false){
            confrimModalOpen('아이디 중복 체크를 완료해 주세요.');
        } else if (비밀번호===''){
            confrimModalOpen('비밀번호를 입력해 주세요.');
        } else if (비밀번호확인===false){
            confrimModalOpen('비밀번호 확인을 완료해 주세요.')
        } else if (이름===''){
            confrimModalOpen('이름을 입력해 주세요.');
        } else if (이메일===''){
            confrimModalOpen('이메일을 입력해 주세요.');
        } else if (isEmailDuplCheck===false){
            confrimModalOpen('이메일 중복 확인을 완료해 주세요.');
        } else if (휴대폰===''){
            confrimModalOpen('휴대폰 번호를 입력해 주세요.');
        } else if (isHpCheck===false){
            confrimModalOpen('휴대폰 인증을 완료해 주세요.')
        } else if (post.주소1===''){
            confrimModalOpen('주소를 입력해 주세요.');
        } else if (이용약관동의필수<3){
            confrimModalOpen('필수 이용약관에 동의해 주세요.')
        } else {
            ...
            newFormData.append('addInput', `${추가입력사항} ${추천인아이디}`);
            newFormData.append('service', 이용약관동의텍스트); // 문자열로 변환
    
            axios({
                url:'http://junelee717.dothome.co.kr/kurly/signup.php',
                method:'POST',
                data:newFormData
            })
            .then((res)=>{
                            ...
                            <ul>
                                ...
                                <li className="row11">
                                    <div className='box1'>
                                        ...
                                        <div className='gender'>
                                            <label><input type="radio" onChange={onChangeGender} checked={state.성별.includes('남자')} name='male' id='male' value="남자"/><em>남자</em></label>
                                            <label><input type="radio" onChange={onChangeGender} checked={state.성별.includes('여자')} name='feMale' id='feMale' value="여자"/><em>여자</em></label>
                                            <label><input type="radio" onChange={onChangeGender} checked={state.성별.includes('선택안함')} name='none' id='none' value="선택안함"/><em>선택안함</em></label>
                                        </div>
                                    </div>                                
                                </li>
                                <li className="row12">
                                ...
                                <li className="row13">
                                    <div className='box1'>
                                        <label>추가입력사항</label>
                                        <div className='gender'>
                                            <label><input type="radio" name='chooChunin' id='chooChunin' value="친구초대 추천인 아이디" checked={추가입력사항.includes("친구초대 추천인 아이디")} onChange={onChangeChooga} /><em>친구초대 추천인 아이디</em></label>
                                            <label><input type="radio" name='event' id='event' value="참여 이벤트명" checked={추가입력사항.includes("참여 이벤트명")} onChange={onChangeChooga}/><em>참여 이벤트명</em></label>                                        
                                        </div>
                                    </div>  
                                </li>   
                                {
                                    ...
                                    (
                                        ...
                                        <li className="row14">
                                        <div className='box1'>
                                            <input type="text" name='userChooga' id='userChooga' maxLength={16} onChange={onChangeChoochun} value={추천인아이디} placeholder={msgChooga1}/>
                                ...
                                <li className="row16">
                                    <div className='box1'>
                                        ...
                                        <div className='service'>
                                            <label><input type="checkbox" name='chkAll' id='chkAll' onChange={onChangeChkAll} value="전체 동의합니다" checked={이용약관동의.length===7}/>전체 동의합니다</label>                                        
                                        </div>
                                    </div>  
                                ...
                                <li className="row17">
                                    <div className='box1'>
                                        <div className='service'>
                                            <label><input type="checkbox" name='chk1' id='chk1' onChange={onChangeChk} value="이용약관 동의(필수)" checked={이용약관동의.includes('이용약관 동의(필수)')}/>이용약관 동의</label><span>필수</span>
                                            <a href="!#">약관보기</a>
                                        </div>
                                    </div>  
                                </li>
                                <li className="row18">
                                    <div className='box1'>
                                        <div className='service'>
                                            <label><input type="checkbox" name='chk2' id='chk2' onChange={onChangeChk} value="개인정보 수집∙이용 동의(필수)" checked={이용약관동의.includes('개인정보 수집∙이용 동의(필수)')}/>개인정보 수집∙이용 동의</label><span>필수</span>
                                            <a href="!#">약관보기</a>
                                        </div>
                                    </div>  
                                </li>
                                ...
                                <div className='box1'>
                                    <div className='service'>
                                            <label><input type="checkbox" name='chk3' id='chk3' onChange={onChangeChk} value="개인정보 수집∙이용 동의(선택)" checked={이용약관동의.includes('개인정보 수집∙이용 동의(선택)')}/>개인정보 수집∙이용 동의</label><span>선택</span>
                                            <a href="!#">약관보기</a>
                                        </div>
                                    </div>  
                                </li>
                                <li className="row20">
                                    <div className='box1'>
                                        <div className='service'>
                                            <label><input type="checkbox" name='chk4' id='chk4' onChange={onChangeChk} value="무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택)" checked={이용약관동의.includes('무료배송, 할인쿠폰 등 혜택/정보 수신 동의(선택)')}/>무료배송, 할인쿠폰 등 혜택/정보 수신 동의</label><span>선택</span>
                                        </div>
                                    </div>  
                                </li>
                                <li className="row21">
                                    <div className='box1'>
                                        <div className='service sms'>
                                            <label><input type="checkbox" name='chk5' id='chk5' onChange={onChangeChk} value="SMS(선택)" checked={이용약관동의.includes('SMS(선택)')}/>SMS</label>
                                            <label><input type="checkbox" name='chk6' id='chk6' onChange={onChangeChk} value="이메일(선택)" checked={이용약관동의.includes('이메일(선택)')}/>이메일</label>
                                        </div>
                                    </div>
                                ...
                                <li className="row22">
                                    <div className='box1'>
                                        <div className='service'>
                                            <label><input type="checkbox" name='chk7' id='chk7' onChange={onChangeChk} value="본인은 만 14세 이상입니다.(필수)" checked={이용약관동의.includes('본인은 만 14세 이상입니다.(필수)')}/>본인은 만 14세 이상입니다.</label><span>필수</span>
                                        </div>
                                    </div> 
                                </li>                            
                            </ul>
